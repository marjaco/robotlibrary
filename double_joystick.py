...
class Double_Joystick:
    def __init__(self, x,y,z,b1,b2):
        self.x = ADC(x)
        self.y = ADC(y)
        self.z = ADC(z)
        self.b1 = Pin(b,Pin.IN, Pin.PULL_UP)
        self.b1.irq(trigger=Pin.IRQ_RISING, handler=self.button_handler)
        self.b2 = Pin(b,Pin.IN, Pin.PULL_UP)
        self.b2.irq(trigger=Pin.IRQ_RISING, handler=self.button_handler)
        self.pressed = False
        self.last_pressed = 0
        self.timer = Timer()
        ...
        self.z_data = deque([conf.JS_Y_MEDIAN,conf.JS_Y_MEDIAN,conf.JS_Y_MEDIAN,conf.JS_Y_MEDIAN,conf.JS_Y_MEDIAN],5)
    ...
    def get_z(self):
        '''This is the y -axis of the secons joystick.'''
        s = self.z.read_u16()
        self.z_data.append(s)
        s = int(sum(self.z_data)/len(self.z_data))
        z = 0
        if s < conf.JS_Y_MEDIAN-300:
            z = abs(conf.JS_MAX_DUTY-s*2)
        elif s > conf.JS_Y_MEDIAN+300:
            z = -abs(conf.JS_MAX_DUTY-s*2)    
        else:
            z = 0
        if z > -1000 and z < 1000:
                z = 0
        return int(conf.MAX_SPEED/conf.JS_MAX_DUTY*z) * conf.JS_REVERSED
...
def main():
    joystick = Double_Joystick(26,27,28,0,1)
