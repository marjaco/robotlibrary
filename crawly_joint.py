...
class Joint:
    ...
    def __init__(self, j_type, name, left_side, inverted, pin):
        ...
        j_type: Short for joint_type. Can be robotlibrary.config.SHOULDER_FRONT, robotlibrary.config.SHOULDER_REAR, 
            robotlibrary.config.KNEE
        ...
        if j_type == robotlibrary.config.SHOULDER_FRONT:
    ...
    def up(self):
        ...
        if self.j_type == robotlibrary.config.KNEE:
            self.servo.set_angle(robotlibrary.config.CRAWLY_UP_ANGLE)
        return False
    def down(self):
    ...
    def forward(self) -> bool:
        ...
        if not self.left_side:
            if self.j_type == robotlibrary.config.SHOULDER_FRONT and self.servo.angle < robotlibrary.config.CRAWLY_FRONT_FORWARD_ANGLE:
                self.servo.set_angle(self.servo.angle +2)
                return True
            if self.j_type == robotlibrary.config.SHOULDER_REAR and self.servo.angle < robotlibrary.config.CRAWLY_REAR_FORWARD_ANGLE:
                self.servo.set_angle(self.servo.angle +2)
                return True
            return False
        else:
            if self.j_type == robotlibrary.config.SHOULDER_FRONT and self.servo.angle > 180-robotlibrary.config.CRAWLY_FRONT_FORWARD_ANGLE:
                self.servo.set_angle(self.servo.angle - 2)
                return True
            if self.j_type == robotlibrary.config.SHOULDER_REAR and self.servo.angle > 180-robotlibrary.config.CRAWLY_REAR_FORWARD_ANGLE:
                self.servo.set_angle(self.servo.angle - 2)
                return True
            return False
    ...
    def backward(self) -> bool:
        '''See the documentation for crawly_leg.py for information.'''
        if not self.left_side:
            if self.j_type == robotlibrary.config.SHOULDER_FRONT and self.servo.angle > robotlibrary.config.CRAWLY_FRONT_BACKWARD_ANGLE:
                self.servo.set_angle(self.servo.angle -2)
                return True
            if self.j_type == robotlibrary.config.SHOULDER_REAR and self.servo.angle > robotlibrary.config.CRAWLY_REAR_BACKWARD_ANGLE:
                self.servo.set_angle(self.servo.angle -2)
                return True
            return False
        else:
            if self.j_type == robotlibrary.config.SHOULDER_FRONT and self.servo.angle < 180-robotlibrary.config.CRAWLY_FRONT_BACKWARD_ANGLE:
                self.servo.set_angle(self.servo.angle + 2)
                return True
            if self.j_type == robotlibrary.config.SHOULDER_REAR and self.servo.angle < 180-robotlibrary.config.CRAWLY_REAR_BACKWARD_ANGLE:
                self.servo.set_angle(self.servo.angle + 2)
                return True
            return False
    ...
    def park(self):
        ...
        elif self.j_type == robotlibrary.config.SHOULDER_FRONT:
            if not self.left_side: 
